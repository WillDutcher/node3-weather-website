...
const forecast = (lat, long, callback) => {
  ...
  request({ url, json: true }, (error, { body }) => {
    if (error) {
      ...
      callback(undefined, 'It is currently ' + f2c(body.current.temperature) + ' degrees. You can expect to see ' + body.current.weather_descriptions[0].toLowerCase() + ' weather today.')
